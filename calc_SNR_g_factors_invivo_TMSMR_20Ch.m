...
%base_folder = '/cluster/tmslab/lucia/TMSMR_28ch_SNR/07292022'
...
sig_path_20Ch = [base_folder,filesep,'rawdata',filesep,'gre_20Ch.dat'];
noise_path_20Ch  = [base_folder,filesep,'rawdata',filesep,'gre_noise_20Ch.dat'];
[snr_cov_20Ch , snr_rss_20Ch , img_20Ch , noisecov_20Ch , noisecorr_20Ch , kspace ] = snr_maps_from_raw_data_3D_jrp(sig_path_20Ch , noise_path_20Ch ,0);
 snr_cov_20Ch  = (snr_cov_20Ch );     snr_rss_20Ch  = (snr_rss_20Ch );

 % specify path to b1 maps (tfl) and to the DICOMs for the gradient echo scan
 % used to acquire the SNR maps (DICOMs are only used for registration)


tfl_20Ch_filepath = [base_folder,'/DICOM/TFL_B1MAP_45SL_CORONAL_2X2X3_0020'];
tfl_20Ch_gre_filepath = [base_folder,'/DICOM/TFL_B1MAP_45SL_CORONAL_2X2X3_0019'];
snr_20Ch_filepath = [base_folder,'/DICOM/GRE_COIL_MAP_2ISO_FULLHEAD_TR50MS_FA90_0017'];
...
dcm2niix_wrapper_l2(tfl_20Ch_filepath,[base_proce_folder,'nifti'],'tfl_20Ch')
dcm2niix_wrapper_l2(tfl_20Ch_gre_filepath,[base_proce_folder,'nifti'],'tfl_20Ch_gre')
dcm2niix_wrapper_l2(snr_with_tms_filepath,[base_proce_folder,'nifti'],'gre_with_tms');
dcm2niix_wrapper_l2(snr_20Ch_filepath,[base_proce_folder,'nifti'],'gre_20Ch');        
...
unix(['cp ',[base_proce_folder,'nifti/gre_20Ch.nii'],' ',[base_proce_folder,'nifti/snr_20Ch.nii']]),
% snr has to be flipped to have same orientation as original data
snr_cov_20Ch_f=fliplr(snr_cov_20Ch(:,:,5:end-4));
nifti_cheat_replace_vol([base_proce_folder,'nifti/snr_20Ch.nii'],squeeze(rot90(snr_cov_20Ch_f,-1)));
...
unix(['cp ',[base_proce_folder,'nifti/snr_20Ch.nii'],' ', [base_proce_folder,'nifti/snr_20Ch_rss.nii']]);
snr_rss_with_tms_f=fliplr(snr_rss_with_tms(:,:,5:end-4));
snr_rss_20Ch_f=fliplr(snr_rss_20Ch(:,:,5:end-4));
nifti_cheat_replace_vol([base_proce_folder,'nifti/snr_20Ch_rss.nii'],squeeze(rot90(snr_rss_20Ch_f,-1)));

nifti_cheat_replace_vol([base_proce_folder,'nifti/snr_with_tms_rss.nii'],squeeze(rot90(snr_rss_with_tms_f,-1)));

%% interpolate flip angle maps into coordinate system of snr maps for 20Ch
...
tfl_20Ch_reg = (load_nifti([base_proce_folder,'nifti/tfl_20Ch_reg.nii.gz']));  tfl_20Ch_reg.vol=(tfl_20Ch_reg.vol);
%% Normalization step  
% No flip angle normalization in this case. 
snr_with_tms_norm = snr_cov_with_tms_f;
snr_20Ch_norm = snr_cov_20Ch_f;
%If NORMALIZATION is required,
% UNCOMMENT THESE LINES!!!  
% snr_with_tms_norm = ((snr_cov_with_tms_f./sin(pi*rot90(tfl_with_tms_reg.vol)/10/180)));
% snr_20Ch_norm = ((snr_cov_20Ch_f./sin(pi*rot90(tfl_20Ch_reg.vol)/10/180)));


snr_with_tms_norm = clean_snr(snr_with_tms_norm);
snr_20Ch_norm = clean_snr(snr_20Ch_norm);
nifti_cheat_replace_vol([base_proce_folder,'nifti/snr_with_tms.nii'],squeeze(rot90(snr_with_tms_norm,-1)));
nifti_cheat_replace_vol([base_proce_folder,'nifti/snr_20Ch.nii'],squeeze(rot90(snr_20Ch_norm,-1)));
  
%%% SNR maps are not registered until here  
  figure(401),imagesc([vol2mos(snr_with_tms_norm) vol2mos(snr_20Ch_norm)],[0 200]),colormap(jet),axis image off
%% register SNR maps for the 20Ch array onto reference case TMSMR 20Ch
%run outside on a shell... problem with format of produced matrix
%use registration_tfl_reg.sh
%unix(['flirt -in ',base_proce_folder,'nifti/snr_20Ch.nii -ref ',base_proce_folder,'nifti/snr_with_tms.nii -o ',base_proce_folder,'nifti/snr_20Ch_reg.nii -cost mutualinfo -v -datatype double'])
%%
temp = (load_nifti([base_proce_folder,'nifti/snr_20Ch_reg.nii.gz']));snr_20Ch_norm_final = rot90(temp.vol,1);%figure(20),imagesc(vol2mos(snr_no_tms_norm_final),[0 1000])
tfl_20Ch_reg_final = (load_nifti([base_proce_folder,'nifti/tfl_20Ch_reg2.nii.gz']));  tfl_20Ch_reg_final=rot90(tfl_20Ch_reg_final.vol,1);
%snr_with_tms_norm_final = (snr_with_tms_norm);
% registere
figure(402),imagesc([vol2mos(snr_with_tms_norm) vol2mos(snr_20Ch_norm_final)],[0 200]),colormap(jet),axis image off
...
img_gfactor_20Ch = img_20Ch(crop2:end-crop2,crop1:end-crop1,:,:);
...
[gfactor_20Ch gfactor_mean_20Ch]=calculate_g_factor((img_gfactor_20Ch(input_crop:end-input_crop,input_crop:end-input_crop+1,:,:)), noisecov_20Ch, sli+offset);


%
figure(103),imagesc([vol2mos((gfactor_with_tms(:,1:1520))); ],[0,1]),colorbar,colormap(jet),axis image off,h=colorbar,set(h,'FontSize',24),daspect([22 10 1]),title('G-factor TMSMR 28CH');

figure(104),imagesc([vol2mos((gfactor_20Ch(:,1:1520))); ],[0,1]),colorbar,colormap(jet),axis image off,h=colorbar,set(h,'FontSize',24),daspect([22 10 1]),title('G-factor 20Ch');
%%
...
figure(106),imagesc([vol2mos((gfactor_20Ch(:,1520:2520))); ],[0,1]),colorbar,colormap(jet),axis image off,h=colorbar,set(h,'FontSize',24),daspect([22 10 1]),title('G-factor 20Ch');
...
save([base_proce_folder,'/snr_mapping_TMSMR_20Ch_comp_April_2023.mat'],'snr_cov_20Ch','snr_20Ch_norm','snr_20Ch_norm_final',...
    'snr_with_tms_norm','snr_rss_with_tms','snr_cov_with_tms','img_with_tms','img_20Ch','noisecov_20Ch', 'noisecorr_20Ch', 'noisecov_with_tms', 'noisecorr_with_tms')
save([base_proce_folder,'/field_mapping_TMSMR_20Ch_comp_April_2023.mat'], 'tfl_with_tms_reg', 'tfl_20Ch_reg','tfl_20Ch_reg_final');
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------'gfactor_with_tms', 'gfactor_20Ch', 'gfactor_mean_with_tms',  'gfactor_mean_20Ch')  
return;
